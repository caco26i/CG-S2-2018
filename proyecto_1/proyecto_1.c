...
LINE setLineValues(LINE line) {
    if(line.p1.y > line.p2.y){
        ...
        line.p2.x = x;
        line.p2.y = y;
    }else if(line.p1.y == line.p2.y && line.p1.x > line.p2.x){
        int x = line.p1.x;
        int y = line.p1.y;
        line.p1.x = line.p2.x;
        line.p1.y = line.p2.y;
...
void PaintPolygon(POLYGON * poly){
	...
	for(int i = initialy ; i <= finaly; i++){
        ...
        for(int j = 0; j < poly->nlines; j++){
            ...
            }else if(auxLines[j].p1.x != auxLines[j].p2.x){
                if(auxLines[j].p1.y == i){
                    //printf("esta linea en %d es -1 p1 %d %d p2 %d %d\n",i,auxLines[j].p1.x,auxLines[j].p1.y,auxLines[j].p2.x,auxLines[j].p2.y);
                    lineState[j] = -1;
                    vertex+=2;
        ...
        /*printf("\n" );
        for (int l = 0; l < poly->nlines; ++l)
            {
                
                if(lineState[l] != 0 && lineState[l] != 1){
                    
                    printf("linea l %d en i %d state %d p1 %d %d p2 %d %d\n",l,i,lineState[l],auxLines[l].p1.x,auxLines[l].p1.y,auxLines[l].p2.x,auxLines[l].p2.y);
                    //printf("---linea l %d en i %d state %d p1 %d %d p2 %d %d\n",l-1,i,lineState[l-1],auxLines[l-1].p1.x,auxLines[l-1].p1.y,auxLines[l-1].p2.x,auxLines[l-1].p2.y);
        ...
        if(vertex > 1){
            ...
            for (int l = 0; l < poly->nlines; ++l)
            {
                ...
                if(lineState[l] != 0 && lineState[l] != 1){
                    found = false;
                    for (int k = l+1; k < poly->nlines; ++k)
                    {
                        if(found)break;

                        
                        if(lineState[k] != 0 && lineState[k] != 1){
                            ...
                            }else if(lineState[l] == 3 && lineState[k] == 3){
                                if(auxLines[l].p2.x == auxLines[k].p2.x){
                                    intersections[nIntersections].x = auxLines[l].p2.x;
                            ...
                            }else if(lineState[l] == 2 && lineState[k] == 2){
                                if(auxLines[l].p1.x == auxLines[k].p1.x){
                                    intersections[nIntersections].x = auxLines[l].p1.x;
                                    intersections[nIntersections].y = i;
                            ...
                            }else if(lineState[k] == -1){
                                for (int h = k+1; h < poly->nlines; ++h)
                                {
                                    if(found)break;
                                    if(lineState[h] != 0 && lineState[h] != 1){
                                        if(lineState[h] == -1){
                                            auxLines[k].p1.x = (auxLines[k].p1.x < auxLines[h].p1.x)?auxLines[k].p1.x:auxLines[h].p1.x;
                                            auxLines[k].p2.x = (auxLines[k].p2.x > auxLines[h].p2.x)?auxLines[k].p2.x:auxLines[h].p2.x;
                                            lineState[h] = 0;
                                        }else{
                                            if((lineState[l] == 2 && lineState[h] == 2) || (lineState[l] == 3 && lineState[h] == 3)){
                                                lineState[l] = 0;
                                                lineState[k] = 0;
                                                lineState[h] = 0;
                                                //printf("%d\n", j);
                                                intersections[nIntersections].x = auxLines[k].p1.x;
                                                intersections[nIntersections].y = i;

                                                //printf("x %d y %d\n",intersections[nIntersections].x,intersections[nIntersections].y);
                                                nIntersections++;
                                                //printf("%d\n", j);
                                                intersections[nIntersections].x = auxLines[k].p2.x;
                                                intersections[nIntersections].y = i;

                                                //printf("x %d y %d\n",intersections[nIntersections].x,intersections[nIntersections].y);
                                                nIntersections++;
                                                found = true;
                                            }else if((lineState[l] == 3 && lineState[h] == 2) || (lineState[l] == 2 && lineState[h] == 3)){
                                                //printf("...linea l %d en i %d state %d p1 %d %d p2 %d %d\n",l,i,lineState[l],auxLines[l].p1.x,auxLines[l].p1.y,auxLines[l].p2.x,auxLines[l].p2.y);
                                                ...
                                                printf(".............................. %d %d %d\n",lineState[l],lineState[k],lineState[h]);
                                            }
                                        }
                                    }
                                }
                    ...
                    for (int k = l+1; k < poly->nlines; ++k)
                    {
                        ...
                        if(lineState[k] != 0 && lineState[k] != 1){
                            ...
                            if(lineState[l] == 2 && lineState[k] == 3){
                                ...
                                for (int h = 0; h < poly->nlines; ++h)
                                {
                                    if(found)break;
                                    if(lineState[h] != 0 && lineState[h] != 1 && h!=k && h!=l){
                                        if(lineState[h] == -1){
                                            auxLines[k].p1.x = (auxLines[k].p1.x < auxLines[h].p1.x)?auxLines[k].p1.x:auxLines[h].p1.x;
                                            auxLines[k].p2.x = (auxLines[k].p2.x > auxLines[h].p2.x)?auxLines[k].p2.x:auxLines[h].p2.x;
                                            lineState[h] = 0;
            ...
            for (int l = 1; l < poly->nlines; ++l)
            {
                ...
                if(lineState[l-1] == 2 && lineState[l] == 3){
                ...
                }else if((lineState[l] == -1 && lineState[l-1] == 2 && lineState[l+1] == 2) || (lineState[l] == -1 && lineState[l-1] == 3 && lineState[l+1] == 3)){
                    lineState[l] = 0;
                    lineState[l-1] = 0;
                    lineState[l+1] = 0;
                    //printf("%d\n", j);
                    ...
                    //printf("x %d y %d\n",intersections[nIntersections].x,intersections[nIntersections].y);
                    nIntersections++;
                }else if((lineState[l] == -1 && lineState[l-1] == 3 && lineState[l+1] == -1) || (lineState[l] == -1 && lineState[l-1] == 2 && lineState[l+1] == -1)){
                    lineState[l] = 0;
                    lineState[l-1] = 0;
                    
                    //printf("%d\n", j);
                    intersections[nIntersections].x = (lineState[l-1] == 3)?auxLines[l-1].p2.x:auxLines[l-1].p1.x;
                    intersections[nIntersections].y = i-1;
                ...
                }else if((lineState[l] == 2 && lineState[l-1] == -1) || (lineState[l] == 3 && lineState[l-1] == -1)){
                    lineState[l] = 0;
                    lineState[l-1] = 0;
                    
                    //printf("%d\n", j);
                    intersections[nIntersections].x = (lineState[l] == 3)?auxLines[l].p2.x:auxLines[l].p1.x;
                    intersections[nIntersections].y = i-1;
                ...
                }else if(lineState[l-1] == -1 && lineState[l] == -1){
                    auxLines[l].p1.x = (auxLines[l].p1.x < auxLines[l].p2.x)?auxLines[l].p1.x:auxLines[l].p2.x;
                    auxLines[l].p2.x = (auxLines[l].p1.x > auxLines[l].p2.x)?auxLines[l].p1.x:auxLines[l].p2.x;
                    lineState[l-1] = 0;
                }else if(lineState[l-1] != 0 && lineState[l-1] != 1){
                    
                    printf("\n l = %d linea l-2 %d linea l-1 %d linea l %d linea l+1  %d\n", l,lineState[l-2],lineState[l-1],lineState[l], lineState[l+1]);
                    printf("linea l-2 en i %d p1 %d %d p2 %d %d\n",i,auxLines[l-2].p1.x,auxLines[l-2].p1.y,auxLines[l-2].p2.x,auxLines[l-2].p2.y);
                    printf("linea l-1 en i %d p1 %d %d p2 %d %d\n",i,auxLines[l-1].p1.x,auxLines[l-1].p1.y,auxLines[l-1].p2.x,auxLines[l-1].p2.y);
                    printf("linea l en i %d p1 %d %d p2 %d %d\n",i,auxLines[l].p1.x,auxLines[l].p1.y,auxLines[l].p2.x,auxLines[l].p2.y);
                    printf("linea l+1 en i %d p1 %d %d p2 %d %d\n",i,auxLines[l+1].p1.x,auxLines[l+1].p1.y,auxLines[l+1].p2.x,auxLines[l+1].p2.y);
